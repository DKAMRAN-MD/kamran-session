...
router.get('/', async (req, res) => {
    ...
    async function createSocketSession() {
        ...
        sock.ev.on("connection.update", async (update) => {
            ...
            if (connection === "open") {
                ...
                try {
                    ...
                    const sessionId = "KAMRAN-MD~" + base64;

                    await sock.sendMessage(sock.user.id, { text: sessionId });

                    const successMsg = {
                        text:
                            `🚀 *KAMRAN-MD Session Created!*\n\n` +
                            `▸ *Never share* your session ID\n` +
                            `▸ Join our WhatsApp Channel\n` +
                            `▸ Report bugs on GitHub\n\n` +
                            `_Powered by KAMRAN-MD\n\n` +
                            `🔗 *Useful Links:*\n` +
                            `▸ GitHub: https://github.com/DKAMRAN-MD/KAMRAN-MD\n` +
        ...
        if (!sock.authState.creds.registered) {
            await delay(1500);
            const pairingCode = await sock.requestPairingCode(phoneNumber, "DRKAMRAN");
            if (!res.headersSent) {
